...
| **Album folder** (`_best-of/`)| Album is featured: shown in a dedicated "Showcase Albums" section on `/albums` with a `★ FEATURED` badge. Photos inside still need their own `_` to be individually featured. |
...
- `GET /` — welcome screen (CRT cycling through showcase, or random fallback)
